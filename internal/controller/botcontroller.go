...
func (c *BotController) CreateBotConfig(ctx *gin.Context) {
	...
	err := c.botService.CreateBotConfig(ctx, req.GroupID, &req.Config)
	if err != nil {
		(&resp.JsonResp{Code: resp.ReFail, Msg: "创建机器人配置失败: " + err.Error()}).Response()
		return
	}

	(&resp.JsonResp{Code: resp.ReSuccess, Msg: "创建机器人配置成功"}).Response()
}
...
func (c *BotController) UpdateBotConfig(ctx *gin.Context) {
	...
	err := c.botService.UpdateBotConfig(ctx, req.GroupID, &req.Config)
	if err != nil {
...
func (c *BotController) GetBotConfig(ctx *gin.Context) {
	...
	configData, err := c.botService.GetBotConfigData(ctx, req.GroupID)
	if err != nil {
